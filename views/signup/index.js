...
exports.signup = function(req, res){
  ...
  workflow.on('validate', function() {
    ...
    else if (!/^(?=.{3,32}$)[a-zA-Z0-9\-\_]+$/.test(req.body.username)) {
    ...
    else if (!/^(?=.{3,40}$)[a-zA-Z0-9\-\_\.\+]+@[a-zA-Z0-9\-\_\.]+\.[a-zA-Z0-9\-\_]+$/.test(req.body.email)) {
      workflow.outcome.errfor.email = 'invalid email format or email is longer than 40 characters.';
    }

    if (!req.body.password) {
      workflow.outcome.errfor.password = 'required';
    }
    else if(!/^(?=.{3,32}$)/.test(req.body.password)) {
      workflow.outcome.errfor.password = 'password must be between 3-32 characters.'
    }
